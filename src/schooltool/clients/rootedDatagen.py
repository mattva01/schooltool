...
$Id: datagen.py 1912 2004-11-03 23:45:30Z jinty $
"""
import random
import sys
import sets
...
def createPupils(nr=nr_pupils):
    ...
    for i in range(nr):
        ...
        names.add(name)
        groups_str = " ".join(("year%d" % year, subject1, subject2, 'root'))
...
def createResources():
    ...
    print >> f, '"%s","locations root"' % _('Hall')
    for i in range(1,10):
        print >> f, '"%s","locations root"' % (_('Room %d') % i)
    for i in range(1,4):
       print >> f, '"%s","root"' % (_('Projector %d') % i)
    f.close()
